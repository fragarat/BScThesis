...
$$\log(\mathbf{I} - \alpha \mathbf{A}) \approx \sum_{k=1}^{n} (-1)^{k+1}\frac{\alpha^k}{k}\mathbf{A}^k$$
...
The Python code used for the three experiments is provided in the appendix \ref{chap:appa} of this thesis. Each experiment involves the definition and analysis of the adjacency matrices generated for the different time intervals. Subsequently, the $\alpha$ and $\beta$ parameters are established to implement the matrix ODE function \eqref{eqn:u3.3}. Such a system of differential equations is then solved using the Scipy's function \texttt{solve\_ivp}, employing Runge-Kutta methods. Furthermore, the thesis includes a comparative analysis of the computational costs associated with the computation of $\mathbf{b}(t)$ and $\mathbf{r}(t)$.
